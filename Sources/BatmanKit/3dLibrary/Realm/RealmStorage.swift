...
open class Storage: StorageProtocol {
    ...
    public  func cachedPlainObjects<T>() -> [T] where T : Translatable {
    ...
    public  func deleteAll<T>(of type: T.Type) throws where T : Translatable {
        let realm = Realm.instance
        let managedObjects = Array(realm.objects(T.ManagedObject.self))
        try realm.write({
    ...
    public func save<T>(objects: [T]) throws where T : Translatable {
        let realm = Realm.instance
    ...
    public func delete<T>(of type: T.Type, byPrimaryKey key: AnyHashable) throws where T : Translatable {
        let realm = Realm.instance
